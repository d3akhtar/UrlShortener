...
function Urls() {
    ...
    if (!isLoading && isSuccess){
        userUrlCodes = data.result.userUrlCodes;
    }
          ...
          <div className='w-75'>
            ...
            </div>
            {
              userUrlCodes!.map((userUrlCode: userUrlCode, i:number) => {
                return i < 10 ? 
                  (<UrlCodeRow urlCode={userUrlCode.urlCode ? (userUrlCode.urlCode):(userUrlCode.alias!)} isAlias={userUrlCode.alias !== undefined} isUserUrlCode={true} key={i}/>):(<></>)
              })
            }
            <div className='row mt-3 w-100 text-start'>
              <div className='col-12 col-md-2'>
                <div className='btn-group d-flex'>
                  ...
                  <button disabled={userUrlCodes!.length < pageSize} onClick={() => setPageNumber(pageNumber + 1)} className='btn btn-light'>{">"}</button>
                </div>
              </div>
