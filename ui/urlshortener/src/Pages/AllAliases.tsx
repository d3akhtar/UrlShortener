...
import { useGetAllAliasesQuery } from '../api/aliasesApi';
...
function AllAliases() {
    ...
    const {data,isLoading,isSuccess,isError,error} = useGetAllAliasesQuery({
        ...
        (
          <div className='w-75'>
            ...
            <div className='row w-100 text-start d-flex justify-content-between'>
              <div className='h2 text-info w-25'>All Aliases</div>
              <form className='w-25'>
                <input 
                  onChange={(e) => {
                    e.preventDefault();
            ...
            </div>
            {
              urlCodes!.map((urlCode: urlCode, i:number) => {
                return <UrlCodeRow urlCode={urlCode} isAlias={true} isUserUrlCode={false} key={i}/>
              })
            }
...
export default withAdmin(AllAliases);